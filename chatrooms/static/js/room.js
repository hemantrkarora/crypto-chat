...
$(function(){
    ...
    var chatSendAction = function (event) {
        if($('#chatSendText').val() !== ""){
            var data_to_send = {
                ...
                "message": Encrypt(escape(getEscapedText($('#chatSendText').val())),document.getElementById('key').value)
    ...
    var getChatText = function(data){
        ...
        for (var i = 0; i < data.length; i++){
            if (latest_message_id < data[i].message_id){
                ...
                to_append.push(Decrypt(unescape(data[i].content),document.getElementById('key').value));
 ...
 var Encrypt = function(plaintext,key){
    //plaintext = document.getElementById("room_data").value.toLowerCase().replace(/[^a-z]/g, "");  
...
var Decrypt = function(ciphertext,key){ 
    ...
    keys = k.split(" "); 
    // do some error checking 
    if(ciphertext.length < 1){ //alert("please enter some ciphertext (letters only, numbers should be spelled)");
	    return; }    
    ...
    if(di == 0){alert("could not invert, try different key "+key); return; }
    ikeys = new Array(4);
    ...
    //$("#log").append('<div class="media-body"><div class="media"><div class="media-body" >'+plaintext'<br /><small class="text-muted">Alex Deo|23rd June at 5:00pm</small><hr /> </div></div></div>');
	return plaintext;
} 
